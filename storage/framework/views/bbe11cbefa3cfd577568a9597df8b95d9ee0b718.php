...
BVTProductManagement.initAddProductForm = function() {
		...
		rules: {
			nameUnique: function(input) {
				...
				if(input.is('input[name="name"]')) {					
					...
					$.post("<?php echo e(action('ProductsController@productNameUnique')); ?>", { name: input.val(), _token: ref._token }, function(r) {
						if($.isEmptyObject(r)) {
							status = true;
						} else {
							status = false;
	...
	$('#addProductForm').submit(function() {
		if(validator.validate()) {
			...
			var url = "<?php echo e(action('ProductsController@store')); ?>";
...
BVTProductManagement.initProductGrid = function() {
	...
	var columns = [
		...
		{ field: 'latest_price', title: 'Latest Price as of <?php echo e(date('M d, Y')); ?>', filterable: false, attributes: { style: 'text-align: right;' } },
